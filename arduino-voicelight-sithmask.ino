...
#define LED_RING 8  // digital port
#define LED_RING_COUNT 16  //LED count
Adafruit_NeoPixel glowring = Adafruit_NeoPixel(LED_RING_COUNT, LED_RING, NEO_GRB + NEO_KHZ800);
...
uint8_t cled=0, cledbrt=0;  //current led reference point and brightness
bool brtdir=true;   //true for increasing brightness, false for decreasing

void loop() {
  
  glowring.setPixelColor(cled, 0, 0, 0);
  glowring.show();
  if (cled >= LED_RING_COUNT) {
    cled=0;
  } else {
    cled++;
  }
  ...
  delay(5000);
  glowring.setPixelColor(cled, cledbrt, 0, 0);
  glowring.show();
  delay(5000);
  Serial.println(cledbrt);

  
//MJ - removed button functionality
/*
  uint8_t c, button;
